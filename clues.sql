...
\c world
SELECT name FROM countries WHERE region = 'Southern Europe' AND population = 1000;
-- Clue #2: Now that we're here, we have insight that 
-- Carmen was seen attending language classes in this country's officially recognized language. 
...
-- our gumshoes tell us she's moved on to a different country, a country where people speak only the language she was learning. 
...
WHERE region = 'Southern Europe' AND language = 'Italian' AND countries.name = cities.name;

-- Clue #5: Oh no, she pulled a switch – 
-- there are two cities with very similar names, but in totally different parts of the globe! 
-- She's headed to South America as we speak; go find a city whose name is like the one we were headed to, 
-- but doesn't end the same. Find out the city, and do another search for what country it's in. Hurry!
...
SELECT district
FROM cities
WHERE name LIKE 'General San Mart%';
